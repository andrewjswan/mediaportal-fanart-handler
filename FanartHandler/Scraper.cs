...
namespace FanartHandler
{
  internal class Scraper
  {
    ...
    public int GetArtistAlbumThumbs(string artist, string album, bool onlyMissing, bool externalAccess, bool onlyClearArt=false)
    {
      var res = 0;
      var flag = true;
      var mbid = (string) null;
          ...
          if (sourceFilename != null && !sourceFilename.Contains("bad_tag")) {
            ...
            if (DownloadImage(ref artist, (category == Utils.Category.MusicAlbumThumbScraped) ? album : null, ref sourceFilename, ref path, ref filename, /*ref requestPic, ref responsePic,*/ category, null)) 
            {
              ...
              if (FanartHandlerSetup.Fh.IsPlaying && !externalAccess) {
                FanartHandlerSetup.Fh.FP.AddPlayingArtistPropertys(FanartHandlerSetup.Fh.CurrentTrackTag, FanartHandlerSetup.Fh.CurrentAlbumTag, FanartHandlerSetup.Fh.FP.DoShowImageOnePlay);
    ...
    public List<string> ExtractURL (string Sec, string AInputString, bool LangIndep = true)
    {
      if (LangIndep || string.IsNullOrEmpty(Utils.FanartTVLanguage))
      ...
      else
        {
          ...
          URLList = ExtractURLLang (Sec, AInputString, Utils.FanartTVLanguage, LangIndep) ;      // Language from Settings
          if ((URLList.Count <= 0) && !Utils.FanartTVLanguageDef.Equals(Utils.FanartTVLanguage, StringComparison.CurrentCulture))
            URLList = ExtractURLLang (Sec, AInputString, Utils.FanartTVLanguageDef, LangIndep) ; // Default Language
    ...
    public int FanartTVGetPictures(Utils.Category category, string id, string artist, string album, int iMax, bool doTriggerRefresh, bool externalAccess, bool doScrapeFanart, string info=null, bool onlyClearArt=false)
    {
      ...
      try
      {
        ...
        #region ClearArt
        // Music
        if (!string.IsNullOrEmpty(Utils.MusicClearArtFolder) && (category == Utils.Category.MusicFanartScraped) && !Utils.GetDbm().StopScraper && Utils.MusicClearArtDownload)
        {
          flag = false;
          if (html != null) {
            if (html.Length > 0) 
          ...
          if (flag)
          {
            ...
            if (DownloadImage(ref artist, null, ref sourceFilename, ref path, ref filename, Utils.Category.FanartTVArt, null))
              logger.Debug("Fanart.TV: Music ClearArt for "+Method+" download complete.");
          }
        }
        // Movies
        if (!string.IsNullOrEmpty(Utils.MoviesClearArtFolder) && (category == Utils.Category.MovieScraped) && !Utils.GetDbm().StopScraper && Utils.MoviesClearArtDownload)
        {
          flag = false;
          if (html != null) {
            if (html.Length > 0) 
          ...
          if (flag)
          {
            ...
            var filename = (string) null;
            var sourceFilename = URLList[0].Substring(checked(URLList[0].IndexOf("|") + 1)) ;
            if (DownloadImage(ref artist, null, ref sourceFilename, ref path, ref filename, Utils.Category.FanartTVArt, id))
        ...
        #region Banner
        // Music
        if (!string.IsNullOrEmpty(Utils.MusicBannerFolder) && (category == Utils.Category.MusicFanartScraped) && !Utils.GetDbm().StopScraper && Utils.MusicBannerDownload)
        {
          flag = false;
          if (html != null) {
            if (html.Length > 0) 
        ...
        // Movies
        if (!string.IsNullOrEmpty(Utils.MoviesBannerFolder) && (category == Utils.Category.MovieScraped) && !Utils.GetDbm().StopScraper && Utils.MoviesBannerDownload)
        {
          flag = false;
          if (html != null) {
            if (html.Length > 0) 
        ...
        #region ClearLogo
        // Movies
        if (!string.IsNullOrEmpty(Utils.MoviesClearLogoFolder) && (category == Utils.Category.MovieScraped) && !Utils.GetDbm().StopScraper && Utils.MoviesClearLogoDownload)
        {
          flag = false;
          if (html != null) {
            if (html.Length > 0) 
        ...
        #endregion
        #region CD
        if (!string.IsNullOrEmpty(Utils.MusicCDArtFolder) && (category == Utils.Category.MusicAlbumThumbScraped) && !Utils.GetDbm().StopScraper && Utils.MusicCDArtDownload)
        {
          flag = false;
          if (html != null) {
            if (html.Length > 0) 
          ...
          if (flag)
          {
            ...
            var filename = (string) null;
            var sourceFilename = URLList[0].Substring(checked(URLList[0].IndexOf("|") + 1)) ;
            if (DownloadImage(ref artist, album, ref sourceFilename, ref path, ref filename, Utils.Category.FanartTVCDArt, null))
              logger.Debug("Fanart.TV: Music CD for "+Method+" download complete.");
          }
        }
        // Movies
        if (!string.IsNullOrEmpty(Utils.MoviesCDArtFolder) && (category == Utils.Category.MovieScraped) && !Utils.GetDbm().StopScraper && Utils.MoviesCDArtDownload)
        {
          flag = false;
          if (html != null) {
            if (html.Length > 0) 
    ...
    private bool DownloadImage(ref string sArtist, string sAlbum, ref string sourceFilename, ref string path, ref string filename, Utils.Category category, string id)
    {
      ...
      {
        if (!Utils.CheckImageResolution(filename, category, false))
        {
          DownloaderStatus = "Stop";
